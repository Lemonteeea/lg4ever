const str = `<p data-nodeid="3223" class="">
...
<p data-nodeid="3231">
在 Network 面板里，我们能看到所有浏览器发起的网络请求，包括页面、图片、CSS
文件、XHR 请求等，还能看到请求的状态（200 成功、404
找不到、缓存、重定向等等）、耗时、请求头和内容、返回头和内容等。
</p>
<p data-nodeid="3232">
图中第一个就是网站页面的请求，返回<code data-backticks="1" data-nodeid="3351"
  >&lt;html&gt;</code
>页面。
</p>
<p data-nodeid="3233">
...
<p data-nodeid="3248">
HTTP 缓存和浏览器渲染原理会分别在第 7 讲和第 8 讲中讲述，今天我们主要围绕 HTTP
请求相关展开。
</p>
...
DNS 的全称是 Domain Name System，又称域名系统，它负责把<code
  ...
  data-nodeid="3367"
  >www.qq.com</code
>这样的域名地址翻译成一个 IP（比如<code data-backticks="1" data-nodeid="3369"
  >14.18.180.206</code
...
<p data-nodeid="3253">
DNS 解析过程会进行递归查询，分别依次尝试从以下途径，按顺序地获取该域名对应的
IP 地址。
</p>
<ul data-nodeid="3254">
<li data-nodeid="3255">
...
<li data-nodeid="3261">
  <p data-nodeid="3262">
    互联网服务提供商 DNS 缓存（联通、移动、电信等互联网服务提供商的 DNS
    缓存服务器）
  </p>
</li>
<li data-nodeid="3263">
...
DNS 服务器进行查找。如果最终依然没找到，浏览器便会将页面响应为打开失败。
</p>
<p data-nodeid="3270">
除此之外，我们在前后端联调过程中也常常需要配置
HOST，这个过程便是修改了浏览器缓存或是系统缓存。通过将特定域名指向我们自身的服务器
IP 地址，便可以实现通过域名访问本地环境、测试环境、预发布环境的服务器资源。
</p>
<p data-nodeid="3271">
那为什么需要配置域名 HOST，而不直接使用 IP 地址进行访问呢？这是因为<strong
  data-nodeid="3387"
...
<li data-nodeid="3279">
  <p data-nodeid="3280">
    ...
    >：利用<code data-backticks="1" data-nodeid="3397">location.hash</code
    >来进行传值。
  </p>
</li>
<li data-nodeid="3281">
  <p data-nodeid="3282">
    使用<code data-backticks="1" data-nodeid="3400">window.name + iframe</code
    >：原理是<code data-backticks="1" data-nodeid="3402">window.name</code
    >值在不同的页面（甚至不同域名）加载后依旧存在。
  </p>
</li>
<li data-nodeid="3283">
  <p data-nodeid="3284">
    使用<code data-backticks="1" data-nodeid="3405">window.postMessage()</code
    >实现跨域通信。
  </p>
</li>
<li data-nodeid="3285">
...
<p data-nodeid="3290">
DNS 解析完成后，浏览器获得了服务端的 IP 地址，接下来便可以向服务端发起 HTTP
请求。目前大多数 HTTP 请求都建立在 TCP 连接上，因此客户端和服务端会先建立起
TCP 连接。
</p>
...
<p data-nodeid="3303">
由客户端发起的 HTTP 请求，服务器收到后会进行回复，回复内容通常包括 HTTP
状态、响应消息等，更具体的会在下一讲 HTTP 协议中进行介绍。
</p>
<p data-nodeid="3304">
...
<p data-nodeid="3311">
因此，目前大多数 HTTP 连接基于 TCP 协议。不过，在 HTTP/3 中底层支撑是 QUIC
协议，该协议使用的是 UDP 协议。因为 UDP 协议丢弃了 TCP
...
寻址过程，以及 IP 协议又是如何将数据包准确无误地传递这些内容，也需要关注
...
<p data-nodeid="3324">
为了避免这种情况，我们开始使用<code data-backticks="1" data-nodeid="3440"
  >异步请求 + 回调</code
>的方式，来进行请求处理，这就是 Ajax。
</p>
<p data-nodeid="3325">
随着时间发展，Ajax 的应用越来越广，如今使用 Ajax 已经是前端开发的基本操作。但
Ajax 是一种解决方案，在前端中的具体实现依赖使用<code
  data-backticks="1"
  data-nodeid="3443"
  >XMLHttpRequest</code
>相关
...
<p data-nodeid="3327">
<code data-backticks="1" data-nodeid="3448">XMLHttpRequest</code>让发送一个
HTTP 请求变得非常容易，我们只需要简单的创建一个请求对象实例，并对它进行操作：
</p>
...
<p data-nodeid="3330">
我们将其封装起来，同时使用 ES6 的<code data-backticks="1" data-nodeid="3460"
  >Promise</code
>的方式，我们可以将其变成一个通过<code data-backticks="1" data-nodeid="3462"
  >Peomise</code
>进行异步回调的请求函数：
</p>
<div class="course-code-area">
